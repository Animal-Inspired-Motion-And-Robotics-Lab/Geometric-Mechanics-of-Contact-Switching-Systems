function traj = ComputeSingleDisk_v2(sim,v)
...
%     v_name = v.v_name; % name of the video
    ...
    alpha_dot = pi/T(1); % the provided time to swing is for the full rotor range
                         % convert this into the constant swing rate
    T_alpha = abs(diff(phi_range,1,1))/(alpha_dot);
    T_beta = T(2)*ones(size(T_alpha)); % the contact time should be the same
    if midF
    ...
    t = linspace(0,phi_tf,floor(phi_tf*frame_r)+1);
    t = t(1:end-1);
...
%     Alpha_r = [1,0]';
    ...
    for i = 1:numL
        ...
        if ~isempty(phi_pin_ord(phi_pin_ord == i)) %%%%%%%%%%%%%%%%%%%%%%%%
            ...
            for j = 1:length(i_time_block)
                ...
                % submanifold.
                if i_time_block(j) > 1
                    prev_time = sum(T_leg(1:i_time_block(j)-1));
                else
                    prev_time = 0;
                end
                next_time = sum(T_leg(1:i_time_block(j)));

                i_swing = ( t_tau < prev_time + T_alpha(i_time_block(j)) )... 
                & ( t_tau >= prev_time );
                ...
                if ~isnan(nextS)
                    ...
                    i_switch = ( t_tau < next_time ) &...
                        ( t_tau >= prev_time + T_alpha(i_time_block(j)) );
                ...
                alpha_curr = interp1(linspace(0,T_alpha(i_time_block(j))),...
                    linspace(phi_range(1,i_time_block(j)),...
                    phi_range(2,i_time_block(j))),...
                    t_tau(i_swing) - prev_time);
    ...
    % Stack vertically
    traj = [Phi_alpha; Phi_beta; Xcm; Ycm; Xpos; Ypos; t];
